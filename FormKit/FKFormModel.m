...
NSString *const FKPickerDidShowNotification = @"com.formkit.picker.show";
NSString *const FKPickerDidHideNotification = @"com.formkit.picker.hide";
...
- (void)showTextViewControllerWithAttributeMapping:
        (FKFormAttributeMapping *)attributeMapping;

- (void)showSelectPickerWithAttributeMapping:
        (FKFormAttributeMapping *)attributeMapping;

- (void)showDatePickerWithAttributeMapping:
        (FKFormAttributeMapping *)attributeMapping;

- (void)showSelectWithAttributeMapping:
        (FKFormAttributeMapping *)attributeMapping;
...
- (id)init {
  ...
  if (self) {
    ...
    self.validationErrorColor = [UIColor colorWithRed:216 / 255.0f
                                                green:98 / 255.0f
                                                 blue:98 / 255.0f
                                                alpha:1];
    self.validationErrorCellBackgroundColor = [UIColor colorWithRed:255 / 255.0f
                                                              green:235 / 255.0f
                                                               blue:235 / 255.0f
                                                              alpha:1];
    self.validationNormalCellBackgroundColor =
...
- (void)didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  FKFormAttributeMapping *attributeMapping =
      [self.formMapper attributeMappingAtIndexPath:indexPath];
  ...
  if (nil != attributeMapping.selectValuesBlock) {
    ...
    FKTextField *textFieldCell =
        (FKTextField *)[self cellForRowAtIndexPath:indexPath];
...
- (FKFormMapping *)mappingForObject:(Class)objectClass {
  ...
  [self.formMappings enumerateObjectsUsingBlock:^(FKFormMapping *mapping,
...
- (void)reloadRowWithIdentifier:(NSString *)identifier {
  ...
  [self.formMapping.attributeMappings
      enumerateKeysAndObjectsUsingBlock:^(id key, FKFormAttributeMapping *
                                                      attributeMapping,
                                          BOOL *stop) {
  ...
  NSAssert(YES == hasReloadedRow,
           @"Identifier doesn't exist, so cannot reload row");
}
...
- (void)reloadRowWithAttributeMapping:
  ...
  NSIndexPath *indexPath =
      [self.formMapper indexPathOfAttributeMapping:attributeMapping];
  ;
...
- (void)reloadSectionWithIdentifier:(NSString *)sectionIdentifier {
  NSString *convertedIdentifier =
      [NSString stringWithFormat:@"section-%@", sectionIdentifier];
  NSUInteger sectionIndex = [self.formMapper.titles
      indexOfObject:[self.formMapping.sectionTitles
                        objectForKey:convertedIdentifier]];
...
- (UIView *)headerWithViewClass:(Class)klass title:(NSString *)title {
  if (klass && [title length] > 0) {
    ...
    if ([headerView conformsToProtocol:@protocol(FKTitleHeaderViewProtocol)]) {
      ...
      CGFloat height = [headerViewWithProtocol
          heightForHeaderConstrainedByWidth:frame.size.width];
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  return [self headerWithViewClass:self.topHeaderViewClass
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  return [self headerWithViewClass:self.bottomHeaderViewClass
                             title:[self titleForFooterInSection:section]];
}
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  if (self.topHeaderViewClass) {
    UIView *header =
        [self headerWithViewClass:self.topHeaderViewClass
                            title:[self titleForHeaderInSection:section]];
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  if (self.bottomHeaderViewClass) {
    UIView *header =
        [self headerWithViewClass:self.bottomHeaderViewClass
                            title:[self titleForFooterInSection:section]];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
  [[[[UIApplication
          sharedApplication] keyWindow] fk_findFirstResponder] resignFirstResponder];
}
...
- (void)showTextViewControllerWithAttributeMapping:
  ...
  Class controllerClass = (attributeMapping.controllerClass == nil)
                              ? self.longTextControllerClass
                              : attributeMapping.controllerClass;
...
- (void)showSelectPickerWithAttributeMapping:
  ...
  ActionStringDoneBlock done = ^(ActionSheetStringPicker *picker,
                                 NSInteger selectedIndex, id selectedValue) {
    FKFormAttributeMapping *formAttributeMapping = picker.formAttributeMapping;
    id value = formAttributeMapping.valueFromSelectBlock(
        selectedValue, self.object, selectedIndex);
    [weakRef.formMapper setValue:value
             forAttributeMapping:formAttributeMapping];
    [weakRef reloadRowWithAttributeMapping:formAttributeMapping];
    [notificationCenter postNotificationName:FKPickerDidHideNotification
                                      object:nil
                                    userInfo:nil];
  };
  ...
  [notificationCenter postNotificationName:FKPickerDidShowNotification
                                    object:nil
                                  userInfo:nil];
  ActionSheetStringPicker *picker;
  picker = [ActionSheetStringPicker
      showPickerWithTitle:attributeMapping.title
                     rows:attributeMapping.selectValuesBlock(value, self.object,
                                                             &selectedIndex)
         initialSelection:selectedIndex
                doneBlock:done
              cancelBlock:cancel
                   origin:(nil == self.viewOrigin) ? self.tableView
                                                   : self.viewOrigin];
...
- (void)showSelectWithAttributeMapping:
  ...
  vc.items =
      attributeMapping.selectValuesBlock(nil, self.object, &selectedIndex);
  vc.title = attributeMapping.title;
  vc.formAttributeMapping = attributeMapping;
  [vc setSlectedIndexPaths:
          [NSArray arrayWithObject:[NSIndexPath indexPathForRow:selectedIndex
                                                      inSection:0]]];

  __weak FKFormModel *weakRef = self;
  [vc setDidSelectBlock:^(NSArray *selectedIndexPaths,
                          BWSelectViewController *controller) {
      ...
      FKFormAttributeMapping *formAttributeMapping =
          controller.formAttributeMapping;
      id value = formAttributeMapping.valueFromSelectBlock(
          selectedValue, self.object, selectedIndex);
      [weakRef.formMapper setValue:value
               forAttributeMapping:formAttributeMapping];
...
- (void)showDatePickerWithAttributeMapping:
  ...
  __weak FKFormModel *weakRef = self;
  actionSheetPicker = [ActionSheetDatePicker
      showPickerWithTitle:attributeMapping.title
           datePickerMode:datePickerMode
             selectedDate:[NSDate date]
                doneBlock:^(ActionSheetDatePicker *picker, NSDate *selectedDate,
                            id origin) {
                    FKFormAttributeMapping *formAttributeMapping =
                        picker.formAttributeMapping;
                    [weakRef.formMapper setValue:selectedDate
                             forAttributeMapping:formAttributeMapping];
                    [weakRef
                        reloadRowWithAttributeMapping:formAttributeMapping];
                }
              cancelBlock:nil
